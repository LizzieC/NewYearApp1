    ...
    for (NSInteger idx = (NSInteger)[downloaders count] - 1; idx >= 0; idx--)
    {
        ...
        if (aDownloader == downloader)
        {
            ...
            if (image)
            {
                ...
                NSDictionary *imgUserInfo = [[downloadInfo objectAtIndex:uidx] objectForKey:@"userInfo"];
                if ([imgUserInfo isKindOfClass:NSNull.class])
                {
                    imgUserInfo = nil;
                }
                ...
                if (imgUserInfo != nil)
                {
                    isCanStore = [[imgUserInfo objectForKey:@"imageIsStore"] boolValue];
                    imgKey = [imgUserInfo objectForKey:@"imageStoreKey"];
                }
            }
            else
            {
