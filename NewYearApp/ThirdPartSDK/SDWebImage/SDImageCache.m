...
static NSInteger cacheMaxCacheAge = 60*60*24*30; // 1 month
...
- (NSString *)cachePathForKey:(NSString *)key imageKey:(NSString *)imgKey
{
    NSString *imgName = @"";
    if (0 != [imgKey length])
    {
    ...
    return [diskCachePath stringByAppendingPathComponent:imgName];
}
...
- (void)storeToDiskWithInfo:(NSArray *)keyAndData
{
    // Can't use defaultManager another thread
    NSFileManager *fileManager = [[NSFileManager alloc] init];
    
    NSString *key = [keyAndData objectAtIndex:0];
    NSString *imgKey = [keyAndData objectAtIndex:1];
    NSData *data = [keyAndData count] > 2 ? [keyAndData objectAtIndex:2] : nil;
    
    NSString *imgSavePath = [self cachePathForKey:key imageKey:imgKey];
    
    if (data)
    {
        [fileManager createFileAtPath:imgSavePath contents:data attributes:nil];
    }
    else
    {
        // If no data representation given, convert the UIImage in JPEG and store it
        ...
        {
#if TARGET_OS_IPHONE
            [fileManager createFileAtPath:imgSavePath contents:UIImageJPEGRepresentation(image, (CGFloat)1.0) attributes:nil];
#else
            NSArray*  representations  = [image representations];
            NSData* jpegData = [NSBitmapImageRep representationOfImageRepsInArray: representations usingType: NSJPEGFileType properties:nil];
            [fileManager createFileAtPath:imgSavePath contents:jpegData attributes:nil];
#endif
            SDWIRelease(image);
        }
    }
    
    NSLog(@"--保存图片:%@",imgSavePath);
...
- (void)notifyDelegate:(NSDictionary *)arguments
{
    ...
    if (image)
    {
        NSLog(@"store to cache (%f)",image.size.height * image.size.width * image.scale);
...
- (void)queryDiskCacheOperation:(NSDictionary *)arguments
{
    ...
    if (info)
    {
        userInfo = [info objectForKey:@"userInfo"];
    }
    NSMutableDictionary *mutableArguments = SDWIReturnAutoreleased([arguments mutableCopy]);

    UIImage *image = SDScaledImageForPath(key, [NSData dataWithContentsOfFile:[self cachePathForKey:key imageKey:[userInfo objectForKey:@"imageStoreKey"]]]);
...
- (void)storeImage:(UIImage *)image imageData:(NSData *)data forKey:(NSString *)key imageKey:(NSString *)imgKey toDisk:(BOOL)toDisk
{
    if (!image || !key || !imgKey)
    {
        return;
    ...
    if (toDisk)
    {
        ...
        if (data)
        {
            keyWithData = [NSArray arrayWithObjects:key,imgKey,data, nil];
        }
        else
        {
            keyWithData = [NSArray arrayWithObjects:key,imgKey, nil];
        }
        
        NSInvocationOperation *operation = SDWIReturnAutoreleased([[NSInvocationOperation alloc] initWithTarget:self
                                                                                                       selector:@selector(storeToDiskWithInfo:)
                                                                                                         object:keyWithData]);
        [cacheInQueue addOperation:operation];
    }
}
...
- (void)removeImageForKey:(NSString *)key imageKey:(NSString *)imgKey fromDisk:(BOOL)fromDisk
{
    if (!imgKey || !key)
    {
        return;
    ...
    if (fromDisk)
    {
        [[NSFileManager defaultManager] removeItemAtPath:[self cachePathForKey:key imageKey:imgKey] error:nil];
    }
}
...
- (void)cleanDiskForCount
{
    NSInteger count = diskMaxCount;
    NSDirectoryEnumerator *fileEnumerator = [[NSFileManager defaultManager] enumeratorAtPath:diskCachePath];
    ...
    for (NSString *fileName in fileEnumerator)
    {
        NSString *filePath = [diskCachePath stringByAppendingPathComponent:fileName];
        NSDictionary *attrs = [[NSFileManager defaultManager] attributesOfItemAtPath:filePath error:nil];
        NSDate *aDate = [attrs fileModificationDate];
        if (aDate)
        {
            [aDict setValue:aDate forKey:fileName];
        }
    }
    
    if ([aDict count] > count)
    {
        NSArray *fileList = [aDict keysSortedByValueUsingSelector:@selector(compare:)];
        for (NSInteger i = 0;i < count;++i)
        {
            NSString *aFilePath = [fileList objectAtIndex:i];
